...
@app.route('/')
def index():
    return render_template('newhome.html',bgimg_url=url_for('static', filename='bgimg.jpg'))

@app.route('/find', methods=['POST'])
def add():
    url = request.form['urlval']
    x=hi(url)
    if x!=0:
        res='Malicious URL'
    else:
        res="Benign URL"
    return render_template('result.html', result=res,given=url)
...
def hi(url):
    df = pd.DataFrame(columns=['Sr. No.', 'Domain Name', 'URL Length', 'Is IP as Host name','Is .exe present',
                               ...
                               'Levenshtein Distance', 'Entropy','Google Search Feature', 'IP Address',
                               ...
                               'Exec() Function', 'Search() function', 'Is image only in webpage', 'Domain Age in Days','Label'
                               ])
    start_time = time.time()
    selfwp=ensure_http_www_prefix(url)
    parsed_url = urllib.parse.urlparse(selfwp)
    hostname = parsed_url.netloc
    # Function Call
    ...
    print("URL Completed in",execution_time,"sec, url:",selfwp)
    res.to_csv("x1.csv", index=False)

    with open('model_pickle_full', 'rb') as f:
        loaded_model = pickle.load(f)
    droplist= ['Sr. No.','Domain Name']
    res = res.drop(columns=droplist)
    res = res.applymap(lambda x: 1 if x is True else (0 if x is False else x))

    #change string features into numeric features
    res['TLD'] = res['TLD'].apply(encoding)
    res['IP Address'] = res['IP Address'].apply(encoding)
    res['ASN Number'] = res['ASN Number'].apply(encoding)
    res['ASN Country Code'] = res['ASN Country Code'].apply(encoding)
    res['ASN CIDR'] = res['ASN CIDR'].apply(encoding)
    res['ASN Postal Code'] = res['ASN Postal Code'].apply(encoding)
    res['ASN creation date'] = res['ASN creation date'].apply(encoding)
    res['ASN updation date'] = res['ASN updation date'].apply(encoding)

    X = res.drop(columns=['Label'])
